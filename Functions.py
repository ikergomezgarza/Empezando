...
def check_years(cashflow, marketcap):
    years = 1
    for i in range(15):
        val = pricing_dcf(cashflow, 0.07, years)
        if val / marketcap > 1:
            print(
...
def check_rate(cashflow, marketcap):
    ...
    r = 0.05
    for i in range(200):
        val = pricing_dcf(cashflow, 0.05, 10, g)
        g += 0.01
        if val / marketcap > 1:
            print(
...
def get_annual_value(facts, concept):
    ...
    key = list(units.keys())[0]
    entries = [
        e for e in units[key] if e.get("form") == "10-K" and e.get("start") is not None
    ]
    entries = [
        e
        for e in entries
        if 300 < (pd.to_datetime(e["end"]) - pd.to_datetime(e["start"])).days < 400
    ]
    if not entries:
        return 0
    entries = sorted(entries, key=lambda x: x["end"], reverse=True)
...
def compare_entries_exits(c_name, params, verbose=False):
    ...
    for i in range(5, 11):
        ...
        for j in range(5, 11):
            ...
            f = returns(c, e, d, exit_multiple=i, years=params.get("years", 5))
...
def get_companys_datas(acq, tgt, verbose=False):
    ...
    if verbose:
        ...
        print("-" * 60)
        print(
            f"{'Market cap':<15} | {round(acq_dict['marketCap']):,} | {round(tgt_dict['marketCap']):,}"
        )
        print(
...
def sensitivity_accretion_dilution(
    acq_dict, tgt_dict, verbose=False, steps=1, **kwargs
):
    ...
    for key in all_dfs_clean:
        ...
        for t in all_dfs_clean[key]:
            try:
                ...
                minority_interest = (
                    get_latest_value(facts, "MinorityInterest")
                    or get_latest_value(facts, "NoncontrollingInterestMember")
                    or 0
                )
                total_cash = get_latest_value(
                    facts, "CashAndCashEquivalentsAtCarryingValue"
                )
                total_revenue = get_latest_value(facts, "Revenues") or get_latest_value(
                    facts, "RevenueFromContractWithCustomerExcludingAssessedTax"
                )
...
def streamlit_df(all_dataframes, ticker):
    ...
    for i in all_dataframes:
        ...
        expected["ev/revenue"] = (
            expected["enterprise value"] / expected["total revenue"]
        )
...
def get_company_dcf_data(ticker, verbose=False):
    ...
    CAPEX = abs(
        get_latest_value(facts, "PaymentsToAcquirePropertyPlantAndEquipment")
        or get_latest_value(facts, "CapitalExpenditures")
        or 0
    )
    ...
    fred = requests.get(
        f"https://api.stlouisfed.org/fred/series/observations?series_id=DGS10&sort_order=desc&limit=1&api_key={FRED_KEY}&file_type=json"
    )
...
def sensitivity_DCF(
    ticker, year=5, equity_risk_premium=0.025, g_longterm=0.025, verbose=False
):
    ...
    for i in g_vals:
        ...
        for j in wacc_vals:
            ...
            enterprise_value = discount_FCF_WACC(
                fcf_list, WACC=j, verbose=False, g_longterm=g_longterm
            )
...
def DCF_Model(ticker, verbose=False, g=0.1):
    ...
    enterprise_value = discount_FCF_WACC(
        fcf_list, WACC, verbose=verbose, g_longterm=0.025
    )
...
class BlackScholes:
    def __init__(self, S, K, T, r, o, call=True):
        ...
        self.d1 = (math.log(self.S / self.K) + (self.r + (self.o**2) / 2) * self.T) / (
            self.o * math.sqrt(self.T)
        )
...
def surface_plotly(surface_df):
    ...
    fig.update_layout(
        scene=dict(
            ...
            yaxis_title="Expiry",
            zaxis_title="IV",
            yaxis=dict(tickmode="array", tickvals=Y, ticktext=pivot.index.astype(str)),
        ),
...
def real_BS_data(ticker):
    ...
    return {"S": S, "r": rate, "days": days, "expiries": expiries, "df": df}
...
class OptionPricing:
    ...
    def payoff(self, spot):
        return sum(
            [self.exit_value(order, spot) * order["trade"] for order in self.orders]
        )
    ...
    def plotgraph(self):
        ...
        x = np.linspace(self.md.S * 0.75, self.md.S * 1.25, 500)
    ...
    def plot_stremlit(self):
        ...
        spots, pnls = self.get_spots()
        S = self.md.S
        x = np.linspace(S * 0.75, S * 1.25, 500)
        y = [self.PnL(spot) for spot in x]
