...
params = {"S": prices[-1], "K": prices[-1], "T": 30, "r": 0.035, "o": vol}
...
with col1:
    ...
    K = st.number_input(
        "Strike price", min_value=0.0, max_value=1000.0, value=prices[-1], step=1.0
    )
    T = st.number_input(
        "Days till expiration", min_value=0, max_value=1000, value=30, step=1
    )
    r = st.number_input(
        "risk free rate", min_value=0.0, max_value=1.0, value=0.035, step=0.005
    )
    o = st.number_input(
        "Asset volatility", min_value=0.0, max_value=1.0, value=vol, step=0.01
    )
...
with col2:
    ...
    fig.add_vline(
        x=S, line_dash="dash", line_color="red", annotation_text="Current Price"
    )
...
with col0:
    st.metric("Price", round(bs.price(), 3))
with col1:
    st.metric("Delta", round(bs.delta(), 3))
with col2:
    st.metric("Gamma", round(bs.gamma(), 3))
with col3:
    st.metric("Theta", round(bs.theta() / 365, 3))
with col4:
    st.metric("Vega", round(bs.vega(), 3))
with col5:
    st.metric("Rho", round(bs.rho(), 3))


st.write("")
chain = opcion_chain(S, K, T, r, o)
st.dataframe(
    chain.style.apply(highlight_in_money, axis=1)
