...
from Functions import (
    get_company_dcf_data,
    forecast_unlevered_FCF,
    get_wacc,
    discount_FCF_WACC,
    final_details,
    sensitivity_DCF,
...
if modify_parameters:
    ...
    g = st.slider("Company growth", 0.0, 0.2, 0.05, step=0.01)
    g_longterm = st.slider("Growth long term", 0.0, 0.2, 0.025, step=0.005)
    equity_risk_premium = st.slider("Equity risk premium", 0.0, 0.2, 0.05, step=0.01)
...
if st.button("Run"):
    with st.spinner("Pulling data from Internet..."):
        ...
        enterprise_value = discount_FCF_WACC(
            fcf_list, WACC, g_longterm=g_longterm, verbose=True
        )
        ...
        if (c_data["share_price"] - intrinsic_price) < 0:
            st.success(
                f"The company is under valued by {round(c_data['share_price'] - intrinsic_price, 2)}$"
            )
        else:
            st.error(
                f"The company is over valued by {round(c_data['share_price'] - intrinsic_price, 2)}$"
            )
        st.write("")
        ...
        st.write(
            f"Cost of equity:           {(c_data['bond_10yr'] + c_data['beta'] * equity_risk_premium):.2%}"
        )
        st.write(
            f"Cost of debt:             {(c_data['interest_expense'] / c_data['total_debt']):.2%}"
        )
        st.write(
            f"Cost of debt after tax:   {((c_data['interest_expense'] / c_data['total_debt']) * (1 - c_data['tax_rate'])):.2%}"
        )
        st.write(
            f"Enterprise value:         {(c_data['market_cap'] + c_data['total_debt']) / 1e6:,.1f}M"
        )
