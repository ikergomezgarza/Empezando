...
choose_expirie = [
    expiries[i] + "  ( " + str(days[i]) + " days left)" for i in range(len(expiries))
]

with col2:
    expirie_selected = st.selectbox("Select expiries", choose_expirie)
...
col1, col2 = st.columns(2)
with col1:
    st.info("Call:       Bid to buy, Ask to sell")
with col2:
    st.info("Put:        Bid to buy, Ask to sell")
...
for i in st.session_state.get("selections", []):
    ...
    st.info(
        f"{trd} a  {i['strike']} {cont} option at {i['premium']}$ that expires in {i['day']} day"
    )
