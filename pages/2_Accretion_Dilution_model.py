...
if modify_parameters:
    ...
    with col1:
        offer_premium = st.slider("Offer premium", 0.0, 1.0, 0.6, step=0.1)
        stock_pct = st.slider("Issued stock percent", 0.0, 1.0, 0.5, step=0.05)
        tax_rate = st.slider("Tax rate", 0.0, 0.50, 0.40, step=0.01)
        interest_rate = st.slider("Interest rate", 0.0, 0.2, 0.10, step=0.01)
        st.write(f"Used Cash percent            {round(1 - stock_pct, 2)}")

    with col2:
        financing_fees_pct = st.slider("Financing Fees", 0.0, 0.1, 0.035, step=0.005)
        transaccion_fees_pct = st.slider("Transaccion Fees", 0.0, 0.1, 0.02, step=0.01)
        synergies_pct = st.slider("Synergies ", 0.0, 0.25, 0.0, step=0.01)
...
if st.button("Run"):
    with st.spinner("Pulling data from Yfinance..."):
        ...
        with col1:
            st.metric(
                "Accretion or dilution per share",
                f"{contract_results['accretion_dilution_per_share']:.2f}",
            )
        with col2:
            st.metric(
                "Acretion or dilution in %",
                f"{contract_results['accretion_dilution_pct']:.2%}",
            )
        ...
        with col1:
            ...
            st.write(
                f"Shares issued:    {contract_results['shares_issued'] / 1e6:,.1f}M"
            )

        with col2:
            st.subheader("Totals:")
            st.write(
                f"Proforma pretax unadjusted:       {fmt(contract_results['profroma_pretax_unadj'])}"
            )
            st.write(
                f"Interest expenses:                {fmt(contract_results['interest_expense_deal'])}"
            )
            st.write(
                f"Amortization of finances fees:    {fmt(contract_results['financing_fees_amort'])}"
            )
            st.write(
                f"Transaccion fees:                 {fmt(contract_results['transaccion_fees'])}"
            )
            st.write(
                f"D/A write off:                    {fmt(contract_results['incremental_DA_expense'])}"
            )
            st.write(
                f"synergies:                        {fmt(contract_results['synergies'])}"
            )
            ...
            st.write(
                f"Proforma pretax adjusted:        {fmt(contract_results['profroma_pretax_adj'])}"
            )
            st.write(
                f"Proforma net income:             {fmt(contract_results['proforma_net_income'])}"
            )
            st.write(
                f"Proforma Shares Outstanding:     {contract_results['proforma_shares_outstanding'] / 1e6:,.1f}M"
            )
            st.write(
                f"Proforma Eps:                    {contract_results['proforma_eps']:,.2f}"
            )
